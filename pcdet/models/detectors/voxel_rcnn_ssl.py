...
class VoxelRCNNSSL(Detector3DTemplate):
    ...
    def forward(self, batch_dict):
        if 'pseudo_label_flag' in batch_dict.keys():
            ...
            batch_dict_train_downsample = copy.deepcopy(batch_dict_train)#xqm
            batch_dict_train_org = copy.deepcopy(batch_dict_train_downsample)
            batch_dict_train_org = self.data_processor.generate_datadict(batch_dict_train_org)

            
          
            
            distance_mask = confident_points[:, 1] < 30.0 #0-70.4
            near_points = confident_points[distance_mask]
            far_points = confident_points[~distance_mask]
            near_points_count = near_points.shape[0]
            target_near_point_count = int(near_points_count / 2.5)
            random_indices = np.random.choice(near_points_count, target_near_point_count, replace=False)

            
            downsampled_near_point_cloud = near_points[random_indices]
            downsampled_confident_points = torch.cat((downsampled_near_point_cloud, far_points), 0)

            batch_dict_train_downsample['points'] = downsampled_confident_points
            batch_dict_train_downsample = self.data_processor.generate_datadict(batch_dict_train_downsample)
            ...
            for cur_module in self.voxel_rcnn.module_list:
                ...
                """ if 'VoxelRCNNHead' == cur_module.model_cfg['NAME']:

                    shared_features = batch_dict_train['shared_features']
                    targets_dict = batch_dict_train['targets_dict'] """
                   
            loss_1, tb_dict, disp_dict = self.get_training_loss()
            for cur_module in self.voxel_rcnn.module_list:
            
                batch_dict_train_downsample = cur_module(batch_dict_train_downsample)
                if 'HeightCompression' == cur_module.model_cfg['NAME']:

                    
                    spatial_features_downsample = batch_dict_train_downsample['spatial_features'].clone()
                    # targets_dict_downsample = batch_dict_train_downsample['targets_dict']

            loss_2, tb_dict_2, disp_dict_2 = self.get_training_loss()

            #we found this easy way also work and can effectively improve training speed
            for cur_module in self.voxel_rcnn.module_list:
                batch_dict_train_org = cur_module(batch_dict_train_org)
                if 'BaseBEVBackbone' == cur_module.model_cfg['NAME']:
                    spatial_features_org = batch_dict_train_org['spatial_features'].clone()

                    lambda_1 = self.se_block(spatial_features_downsample)
                    lambda_2 = self.se_block(spatial_features_org)
                    lambda_1_ = ((lambda_1 / (lambda_1 + lambda_2)).sum())/2
                    lambda_2_ = 1. - lambda_1_

                    mix_feature = spatial_features_downsample * lambda_1_ + spatial_features_org * lambda_2_
                    batch_dict_train_org['spatial_features'] = mix_feature.clone()
            ...
            return pred_dicts, recall_dicts
    @staticmethod
    def f_fusion(feture_1, feature_2):
        new_feature = (feture_1 + feature_2)/2
        return new_feature
    @staticmethod
...
class GetPseudoLabel:
    ...
    def filter_middle_pseudo_label(self, pred_dict, pred_dict_aug, batch_dict, total_rot, total_sca, dual_threshold, label_mask):
        ...
        for bs_idx in range(batch_size):
            ...
            if len(aug_test_box) != 0 and len(test_box) != 0:
